...
#ifdef FLUIDNC
#include "Error.h"
// From https://github.com/bdring/FluidNC/tree/main/FluidNC/src/Error.cpp
...
int maxErrorAlarm() {
  ...
  for (int i = 0; i < (int)ExecAlarm::ExecAlarmEnd; i++) {
    str = alarmString((ExecAlarm)i);
    if (str) {
      if (max < strlen(str))
        max = strlen(str);
    }
...
int maxErrorString() {
  ...
  for (int i = 0; i < (int)Error::ErrorEnd; i++) {
    str = errorString((Error)i);
    if (str) {
      if (max < strlen(str))
        max = strlen(str);
    }
